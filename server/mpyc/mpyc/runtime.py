...
class Runtime:
    ...
    async def start(self):
        ...
        print(f'MPC start is ssl: {self.options.ssl}')
...
#        logging.info(f'Listening on port...')
        ...
        for peer in self.parties[self.pid + 1:]:
            logging.info(f'self.pid = {self.pid}')
            logging.info(f'self.parties = {self.parties}')
            ...
            while True:
                try:
                    ...
                    logging.info(f'Creating connection...')
    ...
    def output(self, x, receivers=None, threshold=None):
        ...
        if x_is_list:
            ...
            x = [x]
        #print(f'isinstance(receivers, int): {isinstance(receivers, int)}, {receivers}')
...
def setup():
    ...
    m = len(parties)
    logging.warning(f'number of parties: {m}')
    print(f'number of parties: {m}')
